...
def evaluate_function(inputs: dict) -> dict:
    result = subprocess.run(["./run-lband.sh",str(inputs["T1"]),str(inputs["T2"]),str(inputs["T3"]),str(inputs["T4"]),str(inputs["T5"])], capture_output=True, text=True)
    #print(result.stdout)
    df = pd.read_csv(
        "LBandOut.txt", skiprows=1, sep=r'\s+',
...
print(X)
X.data.to_csv(data_file, index=False)
while True:
    print(X.data)
