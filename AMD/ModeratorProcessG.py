...
Behavior and outputs are preserved as much as possible.
"""
...
X_HALF: float = 25.0
Y_HALF: float = 25.0
Z_INNER_MIN: float = 1.0
Z_INNER_MAX: float = 58.0
MOD_THICKNESS: float = 0.025  # thickness along normal (mm)
Z_PLATE_CENTERS: Tuple[float, float, float, float] = (18.0, 28.0, 38.0, 48.0)
X_PLATE_CENTERS: Tuple[float, float, float, float] = (+15.0, +5.0, -5.0, -15.0)
Z_EXTENT_CENTER: float = 33.0
Z_EXTENT_HALF: float = 25.0  # So plates cover z in [8, 58]
...
def nearest_surface_distance_mm(end_x: np.ndarray, end_y: np.ndarray, end_z: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Compute distances (mm) from points to nearest moderator/wall surface.

    Returns (dist_overall, dist_to_z_planes, dist_to_x_planes).
    """
    half_t = MOD_THICKNESS / 2.0

    in_xy = (np.abs(end_x) <= X_HALF) & (np.abs(end_y) <= Y_HALF)
    in_y = np.abs(end_y) <= Y_HALF
    in_z_for_xmods = (end_z >= (Z_EXTENT_CENTER - Z_EXTENT_HALF)) & (end_z <= (Z_EXTENT_CENTER + Z_EXTENT_HALF))

    # Z-oriented planes (constant z)
    z_faces = [Z_INNER_MIN, Z_INNER_MAX]
    for c in Z_PLATE_CENTERS:
        z_faces.extend([c - half_t, c + half_t])
    z_faces = np.asarray(z_faces)

    dz_all = np.abs(end_z[:, None] - z_faces[None, :])
    dz_all[~in_xy, :] = np.inf
    dist_z = np.min(dz_all, axis=1)

    # X-oriented planes (constant x): side walls and vertical moderator plates
    dx_walls = np.stack([np.abs(end_x - (+X_HALF)), np.abs(end_x - (-X_HALF))], axis=1)
    dx_walls[~in_y, :] = np.inf

    x_faces = []
    for xc in X_PLATE_CENTERS:
        x_faces.extend([xc - half_t, xc + half_t])
    x_faces = np.asarray(x_faces)

    dx_mods = np.abs(end_x[:, None] - x_faces[None, :])
    valid_xmods = in_y & in_z_for_xmods
    dx_mods[~valid_xmods, :] = np.inf

    dx_all = np.concatenate([dx_walls, dx_mods], axis=1)
    dist_x = np.min(dx_all, axis=1)

    dist_overall = np.minimum(dist_z, dist_x)
    return dist_overall, dist_z, dist_x
...
def plot_energy_fate_breakdown(all_initial_p: Iterable[float],
    ...
    print(len(stopped_ke), len(all_ke), len(stopped_ke) / max(len(all_ke), 1))
