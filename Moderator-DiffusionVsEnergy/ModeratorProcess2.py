...
def fit_and_plot_stopping_distribution(end_z_mm: np.ndarray, *, z_center_mm: float = Z_POSITION, thickness_mm: float = LENGTH, lambda_infinite: bool = False, E: float | None = None) -> None:
    ...
    if lambda_infinite:
        ...
        try:
            ...
            print(f"m, z0, lambda: {popt[0]:.6g} ± {perr[0]:.6g}, {popt[1]:.6g} ± {perr[1]:.6g}, inf")
        except Exception:
            popt = None
    else:
        m0 = 1.9
        ...
        try:
            ...
            perr = np.sqrt(np.diag(pcov))
            print(f"m, z0, lambda: {popt[0]:.6g} ± {perr[0]:.6g}, {popt[1]:.6g} ± {perr[1]:.6g}, {popt[2]:.6g} ± {perr[2]:.6g}")
        except Exception:
            popt = None
