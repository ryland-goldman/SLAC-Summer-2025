...
min_energy = 5
max_energy = 100
energies = np.linspace(min_energy, max_energy, 20)
...
tasks = queue.Queue()
for energy in energies: tasks.put(energy)
...
def run_sum(energy, thickness, angle, threadnumber):
    ...
    try:
        ...
        print(f"Running iteration {i} (Thread {threadnumber}, {energy} MeV)... failed with exception",e)

    print(f"Running iteration {i} (Thread {threadnumber}, {energy} MeV)... done. ({len(positron_E)} events)")
